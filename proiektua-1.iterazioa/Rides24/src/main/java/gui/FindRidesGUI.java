...
public class FindRidesGUI extends JFrame {
	...
	public FindRidesGUI(int param,Rider rider)
	{
		...
		JButton erreserbaButton = new JButton(); //$NON-NLS-1$ //$NON-NLS-2$
		erreserbaButton.setBounds(436, 423, 89, 23);
		
		if(param==1) {
			getContentPane().add(erreserbaButton);
			erreserbaButton.setVisible(true);
			erreserbaButton.setText(ResourceBundle.getBundle("Etiquetas").getString("FindRidesGUI.erreserbatu"));
			erreserbaButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					BLFacade facade=MainGUI.getBusinessLogic();
					String from=jComboBoxOrigin.getSelectedItem().toString();
					String to=jComboBoxDestination.getSelectedItem().toString();
					String driver=(String)tableModelRides.getValueAt(tableRides.getSelectedRow(),0);
					Date date=jCalendar1.getDate();

					Ride ride= facade.findRide(from,to,driver,date);
					if(ride!=null & ride.getnPlaces()>0) {
						System.out.println(ride.toString());
					JFrame a = new BidaiaErreserbatuGUI(ride,rider);
					a.setVisible(true);
					}
				}
				});
